...
<h2><b><center>ADD SWEETS</center></b></h2>
...
<button class="submit" type="submit" name="addsweets">submit</button>
...
?>
<?php
 if(isset($_POST['addsweets'])){
	 $category1=$_POST['category1'];
	 $category2=$_POST['category2'];

	 $sql1="Select PFLAVOUR FROM TADDSWEETS where PID='$category1'";
	 $query1=mysql_query($sql1);
	 $result1=mysql_fetch_array($query1);
	 $flavour=$result1['PFLAVOUR'];
	 $pid="2".$category1.$category2;
	 ...
	 $sql="Select * from TSWEETS where PID='$pid'";
	 $query=mysql_query($sql);
	 $result=mysql_fetch_array($query);
 if($result){
	 $rpavail=$result['PAVAIL'];
		$pavail=$pavail+$rpavail;
		$sql2="update TSWEETS set PAVAIL='$pavail' where PID='$pid'";
	 ...
	 {
		$status="1";
	 $sql1="insert into TSWEETS(PID,PNAME,PPRICE,POPRICE,PQUANTITY,PAVAIL,PIMAGE,STATUS)values('$pid','$flavour','$pprice','$poprice','$quantity','$pavail','$pimg','$status')";
		$query1=mysql_query($sql1);
		echo '<script>alert("PROSUCT ADDED SUCCESSFULLY")</script>';
	 }

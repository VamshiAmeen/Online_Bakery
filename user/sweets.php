...
<div class="container">
    ...
    $sql="Select * from TSWEETS where PNAME like '%$search%'";
		...
		';
				 ...
				 header("Location:doesnotsweet.php");
				}
                            
}else{
		$user=$_SESSION['user'];
				$sql="Select * from TSWEETS where PAVAIL>0";
				$result=mysql_query($sql,$link);
				while($row=mysql_fetch_array($result)){
					$decodedimg=urldecode($row['PIMAGE']);
					$productname=$row['PNAME'];
						 ...
						 quantity:'.$productquantity.'
                          ...
                          <button type="submit" name="add" value='."$productid".' class="btn btn-warning my-3">
                            ...
                            <i class="fas fa-shopping-cart">
						     ...
						     <br>
						   <h7><small><i>sweets incorporate dairy products, such as milk,cream,or butter to achieve a creamy and rich consistency.</i></small></h7>
                       </div>  
                </div>
            </form>
        </div>
		';
				...
				if(isset($_POST['add'])){
					...
					{
						$sql1="Select * from TSWEETS where PID='$pid'";
